...
// Test Python environment endpoint
app.get('/test-python', async (req, res) => {
    try {
        const { spawn } = require('child_process');
        
        const pythonPath = 'python3';
        const pythonScript = spawn(pythonPath, ['./test-python-env.py']);
        ...
        res.status(500).json({
            status: 'error',
            error: error.message,
            message: 'Internal server error during Python test'
        });
    }
});
...
const runMLDetection = (imagePath, latitude, longitude) => {
    return new Promise((resolve, reject) => {
        ...
        const pythonPath = 'python3';
        const pythonScript = spawn(pythonPath, ['./run-solar-panel-and-pool-detection.py', imagePath, latitude, longitude]);
